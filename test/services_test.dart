...
void main() {
  ...
  test("test raw sale_items to sale", () {
    ...
    expect(singleSale.invoice, SaleItem.fromRow(DummyData.raw[0]).invoice, reason: "invoice number should be correct");
    expect(singleSale.quantity, 12, reason: "quantity of first invoice");
    expect(singleSale.items.length,3, reason: "items length");
    
    expect(sales[1].invoice, SaleItem.fromRow(DummyData.raw[4]).invoice, reason: "invoice number should be correct");
    expect(sales[1].quantity, 17, reason: "quantity of first invoice");
    expect(sales[1].items.length,3, reason: "items length");

    expect(sales[0].invoice, SaleItem.fromRow(DummyData.raw[4]).invoice, reason: "invoice number should be correct");
